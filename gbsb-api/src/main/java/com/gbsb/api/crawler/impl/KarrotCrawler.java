...
public class KarrotCrawler extends MediumRssCrawler implements PostCrawler {

    private static final String RSS_FEED_URL = "https://medium.com/feed/daangn";

    private final CompanyRepository companyRepository;

    @Override
    public List<PostDto> crawlPosts() {
        Company company = companyRepository.findCompanyByName(CompanyName.KARROT.getName())
                .orElseThrow(() -> new CrawlException("Karrot company not found in DB"));
        return crawlFromMediumRss(RSS_FEED_URL, company);
    }

    @Override
    public CompanyName getCompanyName() {
        return CompanyName.KARROT;
    }
}
